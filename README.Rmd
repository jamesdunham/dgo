...
Install from GitHub. dgirt requires [RStan](http://mc-stan.org/interfaces/rstan.html) and its
...
Get posterior means with a convenience function, by default for the group means
`theta_bar`:
```{r}
get_posterior_mean(dgirt_out, pars = "theta_bar")
```

Or generally access posterior samples, by default for the group means
`theta_bar`:
```{r}
