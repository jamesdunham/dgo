...
`method = "optimize"` calls `cmdstan optimize`; `method = "variational"` calls `cmdstan variational`. An optional
`algorithm` argument can change the default algorithm from `lbgfs` and `meanfield`, respectively.
