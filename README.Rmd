...
`time_filter` can also be used to include unobserved periods in the model.
...
area. For this toy example we omit the CIs.
...
`data.frame` that just requires a bit more information passed as arguments than
the `dgirtfit` method. Below `poststratify` aggregates over the demographic
grouping variable, so in the subsequent call to `dgirt_plot` we set the
`group_names` argument to `NULL.` 
