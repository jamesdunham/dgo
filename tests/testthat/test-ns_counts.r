suppressMessages({
  ...
  context("Calculate design effects")
  ...
  expect_equal(items_grid$state, as.factor(rep(c("AK", "AL", "AZ"), each = 2, times = 2)))
  expect_equal(items_grid$race, as.factor(rep(c("A", "B"), each = 6)))

  context("Get trial counts")

  items <- dplyr::bind_cols(items, create_gt_variables(items, "item"))
  items$n_responses <- count_respondent_trials(items)
  trial_counts <- summarize_trial_counts(items, items_design_fx, items_grid, items_args)
