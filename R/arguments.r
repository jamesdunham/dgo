...
check_arg_lengths <- function(.arg) {
  ...
  if (any(sapply(length_one, length) == 0)) {
    stop(sprintf("no variable given for %s", stringi::stri_c(c("time_id", "geo_id",
      "survey_weight", "survey_id")[sapply(length_one, length) == 0], collapse = ", ")))
...
check_arg_types <- function(..arg) {
  ...
  if (is.null(..arg$difficulty_count) || !is.numeric(..arg$difficulty_count) ||
    ..arg$difficulty_count < 1 || ..arg$difficulty_count%%1 != 0) {
...
check_no_defaults <- function(..arg) {
  no_default <- ..arg[c("items", "time_id", "groups", "geo_id", "survey_weight",
    "survey_id")]
