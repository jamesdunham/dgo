...
check_order <- function(dgirt_in) {
  ...
  n_vec_groups[, `:=`(group = do.call(paste, c(.SD, sep = "_"))),
               .SDcols = c(dgirt_in$control@group_names)]
  n_vec_groups[, `:=`(group = do.call(paste, c(.SD, sep = "_x_"))),
               .SDcols = c("group", dgirt_in$control@geo_name)]
