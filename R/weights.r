...
create_weights <- function(level1, arg) {
  ...
  check_levels(level1, arg$targets, weight_vars)
...
check_levels <- function(level1, targets, weight_vars) {
  for (s in weight_vars) {
    ...
    class_match <- identical(class(level1[[s]]), class(targets[[s]]))
    if (!class_match) {
      stop("class of '", s, "' differs between item data (", class(level1[[s]]), ") and target data (",
        class(targets[[s]]), ")")
    }
    values_match <- is_subset(level1[[s]], targets[[s]])
    if (!values_match) {
      stop("values of '", s, "' in item data missing from target data: ", paste(setdiff(level1[[s]], targets[[s]]),
          collapse = ", "))
    }
  }
}
