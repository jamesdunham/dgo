...
shape_modgirt = function(data, items, time, geo, groups = NULL, weight = NULL) {
    ...
    ## The result is a dataframe in which each row represents the item response
    ...
    ## could add check to make sure "__wt_tmp__" doesn't yet exist (unlikely)
    stopifnot(!"wt_tmp__" %in% names(opin_long))
...
drop_row_if_all_na = function(.data, varnames) {
    .data[rowSums(is.na(.data[, c(varnames), with = FALSE])) !=
          length(varnames)][]
}
