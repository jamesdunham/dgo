...
namespace model_2017_01_01_namespace {
...
class model_2017_01_01 : public prob_grad {
private:
    int G;
    int G_hier;
    int Q;
    ...
    matrix_d l2_only;
public:
    model_2017_01_01(stan::io::var_context& context__,
        std::ostream* pstream__ = 0)
        : prob_grad(0) {
        typedef boost::ecuyer1988 rng_t;
        rng_t base_rng(0);  // 0 seed default
    ...
    void ctor_body(stan::io::var_context& context__,
        ...
        static const char* function__ = "model_2017_01_01_namespace::model_2017_01_01";
        (void) function__; // dummy call to supress warning
        size_t pos__;
        (void) pos__; // dummy call to supress warning
        std::vector<int> vals_i__;
        ...
        try {
            {
                ...
                lp_accum__.add(lognormal_log<propto__>(disc_raw, 0, 1));
                lp_accum__.add(normal_log<propto__>(sd_gamma_geo, 0, 2.5));
                lp_accum__.add(normal_log<propto__>(sd_gamma_demo, 0, 2.5));
                lp_accum__.add(normal_log<propto__>(sd_innov_delta, 0, 2.5));
                lp_accum__.add(normal_log<propto__>(sd_innov_gamma, 0, 2.5));
                lp_accum__.add(normal_log<propto__>(sd_innov_logsd, 0, 2.5));
                for (int t = 1; t <= T; ++t) {
                    ...
                    if (as_bool(logical_eq(t,1))) {
                        ...
                        lp_accum__.add(normal_log<propto__>(get_base1(sd_theta_bar,t,"sd_theta_bar",1), 0.5, 1));
                        lp_accum__.add(normal_log<propto__>(get_base1(sd_theta,t,"sd_theta",1), 0.5, 2.5));
                        lp_accum__.add(normal_log<propto__>(get_base1(nu_geo,t,"nu_geo",1), 0, 1));
                        lp_accum__.add(normal_log<propto__>(nu_geo_prior, 0, 1));
                        lp_accum__.add(normal_log<propto__>(get_base1(delta_gamma,t,"delta_gamma",1), 0.65000000000000002, 0.25));
                        lp_accum__.add(normal_log<propto__>(get_base1(delta_tbar,t,"delta_tbar",1), delta_tbar_prior_mean, delta_tbar_prior_sd));
    ...
    void write_array(RNG& base_rng__,
        ...
        vars__.resize(0);
        stan::io::reader<double> in__(params_r__,params_i__);
        static const char* function__ = "model_2017_01_01_namespace::write_array";
        (void) function__; // dummy call to supress warning
        // read-transform, write parameters
        vector<vector_d> diff_raw;
        size_t dim_diff_raw_0__ = D;
