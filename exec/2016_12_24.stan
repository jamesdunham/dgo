  ...
  for (t in 1:T) { ## loop over years
    ...
    if (t > 1 && separate_t == 0) {
      if (t == 2) {
       ...
       ## mu_gamma[t] = ZZ_prior[t] * nu_geo_prior; ## 2016-12-24
        for (p in 1:P) {
          if (p <= S) { ## if geographic coefficient
        	mu_gamma[t][p] = ZZ_prior[t][p]*nu_geo_prior;
            gamma[t][p] = mu_gamma[t][p] + sd_gamma_geo * gamma_raw[t][p];
